...
function validContent(value: unknown): value is EditableContent {
  ...
  return Boolean(typeof content.companyName === "string" && content.companyName.trim().length > 1 && /^\d{10}$/.test(content.phone?.replace(/\D/g, "") ?? "") && typeof content.license === "string" && content.license.trim().length > 2 && typeof content.email === "string" && !!content.hero && content.hero.headline.trim().length > 4 && content.hero.description.trim().length > 9 && !!content.seo && content.seo.title.trim().length > 4 && content.seo.description.trim().length > 9 && !!content.ctas && content.ctas.primary.trim() && content.ctas.secondary.trim() && Array.isArray(content.services) && content.services.length > 0 && content.services.every((service) => service.title?.trim() && service.description?.trim() && service.href?.startsWith("/")) && Array.isArray(content.serviceAreas) && content.serviceAreas.length > 0 && content.serviceAreas.every((area) => area.name?.trim() && area.slug?.trim() && area.region?.trim() && typeof area.enabled === "boolean" && area.cityTitle?.trim() && area.metaTitle?.trim() && area.metaDescription?.trim() && area.intro?.trim() && Array.isArray(area.services)) && !!content.acRepair && content.acRepair.heroTitle?.trim() && content.acRepair.heroDescription?.trim() && Array.isArray(content.acRepair.symptoms) && Array.isArray(content.acRepair.diagnostics) && Array.isArray(content.acRepair.repairs) && Array.isArray(content.acRepair.faqs) && !!content.sections);
}
